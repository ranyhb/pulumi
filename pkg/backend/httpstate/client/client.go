...
const (
	...
	JournalingMaxWorkers     = 1
	JournalingPauseTime      = 50 * time.Millisecond
)
...
func (pc *Client) shouldSpawnJournalEntrySpooler() bool {
	...
	if pc.journalSpoolerWorkers >= JournalingMaxWorkers {
		return false
	}
...
func (pc *Client) runJournalEntrySpooler(ctx context.Context, update UpdateIdentifier, token UpdateTokenSource) {
	for {
		time.Sleep(JournalingPauseTime)
		...
		logging.V(1).Infof(
			"Processed a batch of %v entries, %d in flight\n",
			len(batch),
			pc.journalSpoolerWorkers,
		)
	}
